...
from config import (
    PIREXX_BLOCK_SIZE,
    PIREXX_SERVER_ADDRESS,
    PIREXX_UREAD_EXE,
    RUST_PROJECT_DIR,
    client_data_item_dir,
    manifest_path,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Restore a packed file from PIREXX blocks using the client-side manifest.",
    )
    ...
    parser.add_argument(
        "--connect-addr",
        default=PIREXX_SERVER_ADDRESS,
        help="Runtime PIREXX sread address, default comes from config.",
    )
    parser.add_argument(
        "--uread-command",
        help="Explicit pirexx_uread executable path. Defaults to config.PIREXX_UREAD_EXE.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def run_uread(db_name: str, indices: list[int], connect_addr: str, uread_command: Path) -> subprocess.CompletedProcess[str]:
    if not uread_command.is_file():
        raise FileNotFoundError(f"pirexx_uread executable not found: {uread_command}")

    env = os.environ.copy()
    env["PIREXX_TEST_INDICES"] = ",".join(str(index) for index in indices)
    env["PIREXX_N_TEST"] = "1"
...
def read_exported_blocks(indices: list[int], expected_block_size: int) -> bytes:
    directory = client_data_item_dir()
    chunks = bytearray()
...
def write_restored_file(entry: ManifestFile, content: bytes, force: bool) -> Path:
    output_path = resolve_restore_output_path(entry.file_name, allow_collision_suffix=force)
    output_path.write_bytes(content)
    return output_path


def restore_pirexx_file(
    db_name: str,
    file_id: str,
    connect_addr: str = PIREXX_SERVER_ADDRESS,
    uread_command: Path = PIREXX_UREAD_EXE,
    force: bool = False,
) -> dict[str, object]:
    ...
    result = run_uread(db_name, indices, connect_addr, Path(uread_command))
    if result.returncode != 0:
        raise RuntimeError(
            f"pirexx_uread failed for {file_id}\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
...
def main() -> int:
    ...
    result = restore_pirexx_file(
        ...
        uread_command=Path(args.uread_command) if args.uread_command else PIREXX_UREAD_EXE,
        force=args.force,
    )
