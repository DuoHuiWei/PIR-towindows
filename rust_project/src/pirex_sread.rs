...
fn handle_client(storage: & mut Storage, mut stream: TcpStream) -> (Duration, Duration)
{
    ...
    println!("pirex request nbytes {}", full_length);
    ...
    println!("pirex response nbytes {}", result.len());
    println!("pirex response delay {:?} (real measure)", finis - start);
    println!("xor parity comp delay {:?}", t_comp / 2);
...
fn main()
{
    let db_name = env::args().nth(1).expect("usage: pirex_sread <db_name>");
    let listen_addr = env::args().nth(2).expect("usage: pirex_sread <db_name> <listen_addr>");
            ...
            Ok((stream, _)) => 
            {
                ...
                if t_test % n_test == 0
                {
                    let mut file = OpenOptions::new()
                        .create(true)
                        .append(true)
                        .open(state_path("pirex_server_online.txt")).unwrap();
