...
struct RuntimeConfig {
    ...
    server_data_root: String,
    data_dir_name: String,
    data_file_name: String,
    state_dir_name: String,
    raw_item_export_dir: String,
}
...
static NETWORK_ADDRESS: OnceLock<String> = OnceLock::new();

fn find_config_path(scheme: &str) -> PathBuf
{
    if let Ok(path) = env::var("PIR_CONFIG_PATH")
    {
    ...
    let config_name = format!("{scheme}.json");
    ...
    for root in roots
    {
        ...
        {
            let candidate = ancestor.join("config").join(&config_name);
            if candidate.exists()
            {
                return candidate;
            }
    ...
    panic!(
        "config/{} not found from current directory or executable path",
        config_name
    );
}

fn load_runtime_paths(scheme: &str, db_name: &str) -> RuntimePaths
{
    let config_path = find_config_path(scheme);
    let content = read_to_string(&config_path).expect("read runtime config fail");
...
fn extract_json_string(content: &str, key: &str) -> String
{
    ...
    let colon = rest
        .find(':')
        .unwrap_or_else(|| panic!("missing ':' for config key {}", key));
    let mut value = &rest[colon + 1..];
    value = value.trim_start();
    let Some(value) = value.strip_prefix('"') else {
        panic!("config key {} must be a JSON string", key);
    };
    let end = value
        .find('"')
        .unwrap_or_else(|| panic!("unterminated JSON string for key {}", key));
    value[.. end].to_string()
}

fn parse_runtime_config(content: &str) -> RuntimeConfig
{
    RuntimeConfig {
        workspace_root: extract_json_string(content, "workspace_root"),
        server_data_root: extract_json_string(content, "server_data_root"),
        data_dir_name: extract_json_string(content, "data_dir_name"),
        data_file_name: extract_json_string(content, "data_file_name"),
        state_dir_name: extract_json_string(content, "state_dir_name"),
        raw_item_export_dir: extract_json_string(content, "raw_item_export_dir"),
    }
}

fn runtime_paths() -> &'static RuntimePaths
{
    RUNTIME_PATHS.get_or_init(|| {
        let scheme = env::var("PIR_SCHEME").unwrap_or_else(|_| "pirexx".to_string());
        let db_name = env::var("PIR_DB_NAME").expect("PIR_DB_NAME not set; call initialize_runtime or pass db name");
        load_runtime_paths(&scheme, &db_name)
    })
}

pub fn initialize_runtime(scheme: &str, db_name: &str)
{
    env::set_var("PIR_SCHEME", scheme);
    env::set_var("PIR_DB_NAME", db_name);

    if let Some(paths) = RUNTIME_PATHS.get()
    {
        if paths.scheme != scheme || paths.db_name != db_name
        {
            panic!(
                "runtime already initialized for scheme={} db={}, cannot switch to scheme={} db={}",
                paths.scheme,
                paths.db_name,
                scheme,
                db_name
    ...
    let paths = load_runtime_paths(scheme, db_name);
    let _ = RUNTIME_PATHS.set(paths);
}

pub fn initialize_network_address(address: &str)
{
    env::set_var("PIR_SERVER_ADDRESS", address);

    if let Some(current) = NETWORK_ADDRESS.get()
    {
        if current != address
        {
            panic!("network address already initialized as {}, cannot switch to {}", current, address);
        }
        return;
    }

    let _ = NETWORK_ADDRESS.set(address.to_string());
}

pub fn server_address() -> String
{
    if let Some(address) = NETWORK_ADDRESS.get()
    {
        return address.clone();
    }

    env::var("PIR_SERVER_ADDRESS").unwrap_or_else(|_| "127.0.0.1:8111".to_string())
}

pub fn db_name() -> &'static str
{
    &runtime_paths().db_name
}
...
{
    PathBuf::from(&runtime_paths().config.workspace_root)
}
...
pub fn database_root() -> PathBuf
{
    ...
    workspace_root()
        .join(&paths.config.server_data_root)
        .join(&paths.db_name)
}
...
{
    let paths = runtime_paths();
    database_root().join(&paths.config.state_dir_name)
}
...
{
    let paths = runtime_paths();
    database_root().join(&paths.config.data_dir_name)
}

pub fn ensure_data_dir()
{
    create_dir_all(data_dir()).expect("create data dir fail");
}

pub fn raw_item_export_dir() -> PathBuf
{
    let paths = runtime_paths();
    workspace_root().join(&paths.config.raw_item_export_dir)
}

pub fn ensure_raw_item_export_dir()
{
    create_dir_all(raw_item_export_dir()).expect("create raw item export dir fail");
}

pub fn raw_item_export_path(index: usize) -> PathBuf
{
    raw_item_export_dir().join(format!("block_{index}.bin"))
}
...
{
    let paths = runtime_paths();
    data_dir().join(&paths.config.data_file_name)
}
...
pub fn ensure_parent_dir(path: &Path)
{
    ...
    {
        create_dir_all(parent).expect("create parent dir fail");
    }
}
