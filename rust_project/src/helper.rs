...
fn init_state_dir()
{
    ...
    println!("helper: runtime config {:?}", config_file_path());
    println!("helper: using database {}", db_name());
    println!("helper: data path {:?}", source);
...
fn main()
{
    ...
    let scheme = args.next().unwrap_or_else(|| "pirexx".to_string());
    let db_name = args.next().expect("usage: helper <scheme> <db_name> [init-state|init-all]");
    let command = args.next();

    initialize_runtime(&scheme, &db_name);
